...
The Dashboard and Actions page display the stored expense name or reference so reviewers can keep related files from the same multi-file upload together end to end. Dashboard shows an Expense groups overview and renders multi-file phase queues as compact expense/reference cards with one `Review` button, a stage summary, and a collapsed `Show files` section for the detailed file list. The group `Review` action selects the best next actionable file first, preferring compliance review and approval work before retry, active, or reviewed records. Actions shows linked files for the same expense/reference beside the selected document. The Actions selector is labeled `Selected file for review`, and each option includes the exact file name, document ID, expense/reference, stage, and upload time so duplicate or similar file names are easier to distinguish. The selected record header repeats the file name and shows a selected-file notice with document ID, linked-file count, and current action. For multi-file expense/reference groups, Actions also shows group aggregation: total files, files needing decision, files needing fix, total extracted items/services, total risk notes, and an Items / Services by file table when line items are present. The Actions page places the Decision panel near the top, before source download and the full AI analysis, so reviewers can correct type, approve, or reject without scrolling. It uses the preserved local working copy in `data/uploads` and shows a `Download Doc for Review` button so the reviewer can open the original file locally. If the working copy is missing, the reviewer still sees metadata, lifecycle details, extracted text, and generated analysis.
...
15. Upload or simulate a public-sector expense and confirm the correct small, medium, or high compliance attention risk is added.
```
